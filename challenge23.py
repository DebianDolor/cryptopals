...
def get_bit(number, position):
    # returns the bit at the given position of the given number (left to right)
...
def set_bit_to_one(number, position):
    # sets the bit at the given position of the given number to 1
...
def undo_right_shift_xor(result, shift_len):
    ...
    are the same as the original value before the operation 
    (because when we shift the input to the right its first bits will be zeros, 
    and when we XOR something against zeros, that something does not change).

    The following bits will instead be equal to the XOR of the original bit at their same
    position and the original bit that was "shift_len" positions behind. 

    We can recover the original bit at their position by XORing the resulting bit
    with the original bit "shift_len" positions behind
    """
    original = 0
    for i in range(32):
...
def undo_left_shift_xor_and(result, shift_len, andd):
    ...
    by redoing the AND between the un-shifted resulting value and the and'd value
...
def untemper(y):
    """
    Reverts the tamper process when extract_number() is called, 
    and returns the initial value state of the generator corresponding
