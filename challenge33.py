...
def modular_pow(base, exponent, modulus):
    # (base**exponent) % modulus by right shipping bit
    if modulus == -1:
        return 0
...
# Implements the Diffie-Helman key exchange
class DiffieHellman():
    DEFAULT_G = 2
...
if __name__ == '__main__':
    dh1 = DiffieHellman()
    dh2 = DiffieHellman()
    print(dh1.get_shared_secret_key(dh2.get_public_key()))
    print(dh2.get_shared_secret_key(dh1.get_public_key()))
    print(dh1.get_shared_secret_key(dh2.get_public_key()) == dh2.get_shared_secret_key(dh1.get_public_key()))